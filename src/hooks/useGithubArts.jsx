...
export const useGitHubArts = () => {
  ...
  const buscarArtes = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      // Buscar pasta principal da galeria
      ...
      await Promise.all(
        pastasDesafio.map(async (pasta) => {
          try {
            ...
            if (!respArquivos.ok) {
              console.warn(`Erro ao buscar pasta ${pasta.name}: ${respArquivos.status}`);
              return;
            }
            ...
            imagens.forEach((img, index) => {
              const nomeArquivo = img.name.replace(/\.(jpg|jpeg|png|gif|webp)$/i, '');
